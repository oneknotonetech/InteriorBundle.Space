// app/admin/gen-ai-space/page.tsx
'use client'
...
const AdminDashboard: React.FC = () => {
  ...
  const downloadAllImages = (submission: UserSubmission) => {
    submission.inspirationImages.forEach((image, index) => {
      downloadImage(image.url, `inspiration_${index + 1}_${image.name}`);
    });
    
    submission.areaImages.forEach((image, index) => {
      downloadImage(image.url, `area_${index + 1}_${image.name}`);
    });
  };
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="mb-4">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => downloadAllImages(submission)}
